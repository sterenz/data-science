...
class GenericQueryProcessor(object):
    ...
    def getPublicationsPublishedInYear(self, _year: int) -> list[Publication]:
        ...
        if not relational_publication_published_year_df.empty :
            ...
            # Rename Data Frame columns.
            relational_publication_published_year_df.rename(
                    columns = {
                        'id'              : 'publication_doi', 
    ...
    def getMostCitedVenue(self) -> Venue:
        ...
        if relational_most_cited_venue_df.empty:
            # Relational DataFrame Clean up.
            #relational_most_cited_venue_df.drop(["venue", "noOfCites"], axis=1, inplace=True)
    ...
    def getVenuesByPublisherId(self, _id: str) -> list[Venue]:
        ...
        if not relational_venues_by_publisher_id_df.empty:
            # Relational Dataframe clean up.
            relational_venues_by_publisher_id_df.drop(["id"], axis=1, inplace=True) # Since two columns are named id both are dropped.
            
            # Rename Data Frame columns.
            relational_venues_by_publisher_id_df.rename(
                    columns = {
                        'id'  : 'venue_id', 
                        'name': 'venue_title', 
                    }, 
    ...
    def getJournalArticlesInIssue(self, _issue: str, _volume: str, _journalId: str) -> list[JournalArticle]:
        ...
        if not relational_journal_articles_in_issue_df.empty:
            # Relational DataFrame clean up.
            #relational_journal_articles_in_issue_df.drop(["JournalArticle", "journal", "venueId"], axis=1, inplace=True)
    ...
    def getJournalArticlesInVolume(self, _volume: str, _journalId: str) -> list[JournalArticle]:
        ...
        if not relational_journal_articles_in_volume_df.empty:
            # Relational DataFrame clean up.
            relational_journal_articles_in_volume_df.drop([], axis=1, inplace=True)
                
            # Rename Data Frame columns.
            relational_journal_articles_in_volume_df.rename(
                    columns = {
                        'JournalArticleId'             : 'journal_article_doi', 
                        'JournalArticleTitle'          : 'journal_article_title', 
                        'JournalArticlePublicationYear': 'journal_article_publication_year', 
    ...
    #TODO: implement!.
    # getProceedingsByEvent(_eventPartialName: string): list[Proceedings].
    ...
    def getDistinctPublisherOfPublications(self, _pubIdList: list[str]) -> list[Organization]:
        ...
        if isinstance(_pubIdList,list):
            ...
            result_str: str = '(' # Init the result string.
        ...
        for processor in self.queryProcessor:
            # Type guard.
            if isinstance(processor, TriplestoreQueryProcessor):
                triplestore_publishers_df = processor.getDistinctPublisherOfPublications(result_str)
            elif isinstance(processor, RelationalQueryProcessor):
        ...
        if not relational_publishers_df.empty:
            ...
            # Rename Data Frame columns.
            relational_publishers_df.rename(
                    columns = {
                        'organizationId'   :'organization_id', 
                        'organizationName' :'organization_name', 
                    }, 
